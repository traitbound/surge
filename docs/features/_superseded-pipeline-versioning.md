...
**Staleness.** Creating a revision means the project's graph has moved since its last compile, which is what `PipelineAssignmentStatus::Stale` used to document itself as. **Citation superseded 2026-08-28 by ESC-3 (`1eb48dc`):** that variant no longer exists — the enum is now `Published | NotCompiled`, derived from materialization freshness, because `Stale` was itself unproducible (`insert_fresh` always inserts a successor in the same transaction). A genuine `Stale` becomes producible exactly when `pipeline-revisions` lands the "mark `fresh = 0` in the revision transaction" write this paragraph describes; the split predicate `EXISTS(any) AND NOT EXISTS(fresh)` is recorded in the enum's own doc. So a revision write marks the project's materialization `fresh = 0` in the same transaction. Without this, dispatch keeps running the published graph while the operator edits a different one.
...
| **`pipeline.content_hash` is an unmaintained placeholder** | `crates/domain/src/fixtures.rs:15-16` | ~~the only pipeline row in the tree carries the literal `"sha256:fixture-two-node-v1"`… AC 8 fixes this~~ — **FIXED 2026-08-28 by ESC-1 (`3ecd585`), outside this spec.** The claim was true when written; the seed now derives the hash before insert. Successor specs must not re-scope it |
...
| ~~`PipelineAssignmentStatus::Stale` already means "the graph moved"~~ **— claim retired by ESC-3 (`1eb48dc`)** | `crates/domain/src/project.rs` | the variant was removed; the enum is `Published \| NotCompiled`, derived from materialization freshness. `pipeline-revisions` re-adds `Stale` when it makes staleness producible |
