...
10. Claude Code plugin: full MCP tool surface (work-order fetch, lease claim) added to the Phase 0 skeleton (design §23-Eighteen). **Includes making the loop depend on the fetch** (smoke walk 6, R2): today `crates/server/src/supervisor.rs:389-394` inlines the entire rendered work order into the worker prompt and merely suggests the tool, so the call is discretionary — 1 of 3 real workers made it. The prompt should carry a pointer plus the lease/materialization hash and let the worker fetch the authoritative copy, which is what INV-ID-1's "authoritative statement of the task" actually describes.
