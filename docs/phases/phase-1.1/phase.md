...
Relocation is `role:critical` (code map, compiler row: hash-input changes are serialized), so it is not a drive-by. **Being done now as ESC-4** (dispatched 2026-08-29), ahead of `pipeline-revisions`, because that spec adds the second hash writer and should not inherit the decision unresolved. `pipeline-assignment` reads `content_hash` through a join and adds no writer, so it never forced it.
