...
### Instance settings — *640px column · five cards*
...
| MODEL ROUTING | Primary models come from the pipeline's roles; this card is only about failure. A fallback model used after two failed retries on 429/5xx, and per-run cost caps for coordinator, worker and verifier. The consequence is stated: "A cap breach pauses the wave and queues a required gate — the same path as the budget breaker." |
