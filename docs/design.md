...
>
> **Eight — resolved: hash inputs are defined by rule (INV-ID-2).** Semantic content only — node and edge **ids**, kinds, configs, gates, fanout, triggers, and pinned references. Positions, labels, frames, stickies, collapse state and the observability fields (`emits_span`, `metric_binding`, `metric_note`) never enter the hash; the known-lossy annotation round-trip therefore cannot break canvas↔code hash fidelity. *(2026-08-28: "prompts" struck — prompt bodies never reach the pipeline hash. They enter identity transitively, via pinned `LibraryRef` + INV-DATA-2 immutability, and directly at the materialization hash, which covers emitted bytes. See INV-ID-2's dated amendments.)*
>
...
> **Twenty-Two — recorded (2026-08-28): the project pipeline-status enum is temporarily narrower than this document describes.** This doc declares the field as `published | stale` (§03 Project) and describes a stale badge and a "dispatch refused — stale materialization" NEEDS-YOU row. The shipped enum is `published | not_compiled`. Reason: nothing ever wrote `stale` — the badge was driven by a column no code set — and `stale` was *unproducible* besides, because `materializations::insert_fresh` is the only writer of `materialization.fresh` and always inserts a successor in the same transaction, so "rows exist, none fresh" could not occur. Rather than leave a badge asserting a compile that never happened, ESC-3 (`1eb48dc`) derived the status from materialization freshness and renamed the variant to what the system can actually evidence. **This is drift, not contradiction:** `stale` returns — additively, to a two-variant enum — when `pipeline-revisions` lands the write that marks a materialization stale on a project-local edit, at which point the split predicate is `EXISTS(any) AND NOT EXISTS(fresh)`. Until then, this document describes the target state and the code describes the reachable one.
