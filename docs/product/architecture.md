...
- *SQLite plus a derived SurrealDB recall store* — pays the embedded-SurrealDB build cost anyway while keeping two transaction boundaries; the divergence it permits is exactly what INV-DATA-8 exists to forbid.
