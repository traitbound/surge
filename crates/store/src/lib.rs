...
mod object_model_tests {
    ...
    /// R1 (smoke walk 6): the supervisor's observability floor needs to ask
    /// what a run's spans *said*, not just how many there were. Severity
    /// order, and the exclusion that makes the answer the worker's verdict
    /// rather than a reading-back of the supervisor's own termination record.
    #[tokio::test]
    async fn worst_span_status_ranks_severity_and_ignores_reserved_ids() {
        // The prefixes the server reserves (`supervisor::RESERVED_SPAN_PREFIXES`).
        const RESERVED: [&str; 5] =
            ["sp_end_", "sp_fail_", "sp_orphan_", "sp_abort_", "sp_run_"];
        let pool = crate::open_in_memory().await.unwrap();
        seed_project_and_pipeline(&pool).await;

        for (case, spans, want) in [
            ("no spans at all", vec![], None),
            ("only ok", vec![("sp_w", SpanStatus::Ok)], Some(SpanStatus::Ok)),
            (
                "an unclosed span outranks ok but is not an error",
                vec![("sp_w", SpanStatus::Ok), ("sp_open", SpanStatus::Running)],
                Some(SpanStatus::Running),
            ),
            (
                "refused outranks running",
                vec![("sp_open", SpanStatus::Running), ("sp_no", SpanStatus::Refused)],
                Some(SpanStatus::Refused),
            ),
            (
                "error outranks everything, whatever else the run recorded",
                vec![
                    ("sp_w", SpanStatus::Ok),
                    ("sp_open", SpanStatus::Running),
                    ("sp_no", SpanStatus::Refused),
                    ("sp_bad", SpanStatus::Error),
                ],
                Some(SpanStatus::Error),
            ),
            (
                "every reserved id is invisible, even carrying error",
                vec![
                    ("sp_w", SpanStatus::Ok),
                    ("sp_end_x", SpanStatus::Error),
                    ("sp_fail_x", SpanStatus::Error),
                    ("sp_orphan_x", SpanStatus::Error),
                    ("sp_abort_x", SpanStatus::Error),
                    ("sp_run_x", SpanStatus::Refused),
                ],
                Some(SpanStatus::Ok),
            ),
            (
                "a run with nothing but supervisor records has no verdict",
                vec![("sp_abort_x", SpanStatus::Error)],
                None,
            ),
        ] {
            let run_id = format!("run_{}", case.replace(' ', "_"));
            crate::observatory::insert_run(&pool, &Run {
                id: run_id.clone(),
                project_id: "prj_fixture".into(),
                issue_id: None,
            ...
            .await
            .unwrap();
            for (i, (id, status)) in spans.iter().enumerate() {
                crate::observatory::append_span(&pool, &Span {
                    id: format!("{id}_{run_id}"),
                    run_id: run_id.clone(),
                    parent_span_id: None,
                    node_id: None,
            ...
            let got = crate::observatory::worst_span_status(&pool, &run_id, &RESERVED)
                .await
                .unwrap();
